...
def find_todo_with_adapter_for(api: str, adapter: Callable[[str], dict]) -> dict:
...
def retrieve_todo_with_adapter(
    api: str, adapter: Callable[[str], dict] = requests_adapter
) -> TypeList[LiteDbTodoItem]:
    """Retrieve todo implementation that uses an adapter."""
    data = find_todo_with_adapter_for(api, adapter=adapter)
    data_len = len(data)
    items = []
...
def requests_adapter_post(url: str, body, header) -> dict:
    """An adapter that encapsulates requests.get"""
...
def post_todo_with_adapter_for(api: str, post_adapter: Callable[[str, Any, Any], dict]) -> dict:
    """Find the todo using an adapter."""
    # url = API.format(city_name=city, api_key=API_KEY)
    url = api
    headers, body = {}, {}
    ...
    return post_adapter(url, body, headers)


def post_todo_with_adapter(
    api: str, post_adapter: Callable[[str, Any, Any], dict] = requests_adapter
) -> LiteDbTodoItem:
    """Retrieve todo implementation that uses an adapter."""
    data = post_todo_with_adapter_for(api, post_adapter=post_adapter)
    return LiteDbTodoItem.from_dict(data)
